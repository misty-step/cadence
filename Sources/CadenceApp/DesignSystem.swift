...
enum DesignSystem {
    ...
    enum Gradients {
        ...
        static func background(for phase: TimerState.Phase, scheme: ColorScheme) -> LinearGradient {
            let stops: [Color]
            switch (phase, scheme) {
            case (.focus,      .light): stops = focusLight
            case (.shortBreak, .light): stops = shortBreakLight
            case (.longBreak,  .light): stops = longBreakLight
            case (.focus,      .dark):  stops = focusDark
            case (.shortBreak, .dark):  stops = shortBreakDark
            case (.longBreak,  .dark):  stops = longBreakDark
            @unknown default:           stops = focusLight
            }
    ...
    enum Typography {
        static func phaseLabel() -> Font  { Font.custom("Outfit", size: 11).weight(.medium) }
        static func timeDisplay() -> Font { Font.custom("Outfit", size: 84).weight(.light) }
        static func buttonLabel() -> Font { Font.custom("Outfit", size: 14).weight(.medium) }
    }
