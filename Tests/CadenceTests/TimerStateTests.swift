...
struct TimerStateTests {
    ...
    @Test func shortBreakToFocus() {
        ...
        state.start()
        state.secondsRemaining = 1
        state.tick() // focus -> shortBreak

        state.secondsRemaining = 1
        state.tick() // shortBreak -> focus
    ...
    @Test func longBreakToFocus() {
        ...
        state.start()
        state.secondsRemaining = 1
        state.tick() // focus -> longBreak

        state.secondsRemaining = 1
        state.tick() // longBreak -> focus
    ...
    // MARK: - onPhaseChange Callback

    @Test func onPhaseChangeCalledOnAdvance() {
        let state = TimerState()
        var receivedPhase: TimerState.Phase?
        state.onPhaseChange = { phase in
            receivedPhase = phase
        }
        state.start()
        state.secondsRemaining = 1
        ...
        #expect(receivedPhase == .shortBreak)
    }
